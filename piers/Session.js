document.currentScript.value = (async function(SE){
	...
	class HomeBase { // {{{
		...
		async request (url, payload) {
			return await Piers.fetch(url,payload)
		}
	}	// HomeBase }}}
	...
	class RSAHome extends HomeBase { // {{{
			...
			try {
				let self=this,
					pubKey=new Piers.Session.PublicKey(
						await Piers.fetch(self.Opts.API.PREFIX + self.Opts.API.GETKEY)
					);
				...
				let result=await Piers.fetch(
					self.Opts.API.PREFIX + ( enroll ? self.Opts.API.ENROLL : self.Opts.API.AUTH ), {
						"U": user,
		...
		async request (url, payload) {
			let self=this;
			url=await Piers.fetch(self.Opts.API.PREFIX+url,payload);
			if (self.Keys.Profile) {
	...
	class MSHome extends HomeBase { // {{{
		...
		async authenticate (user, pass, enroll=false) {
			let pf = await this.request(this.Opts.API[enroll ? "ENROLL" : "LOGIN"], {
				"U":user,
				"S":pass
			})
			this.enter(pf.D);
		...
		async request (url, payload){
			return await Piers.fetch(this.Opts.API.PREFIX+url, payload);
		}
	}	// MSHome }}}
	...
	let X = await Piers.Promise([
		Piers.import({"xl":"sha"}),
		Piers.import({"xl":"jsencrypt"})
	]);
	return (Session = {
		"SHA": X.sha512,
		"SHA_b64": (s)=>btoa(
