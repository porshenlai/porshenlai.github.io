	...
	<div data-uid='aside:tab:Settings'>
		...
		<div data-uid='aside:Settings:FontSize'>
			...
			<div style='display:flex;flex-flow:row nowrap;'>
				<input data-h='page:resize' type='range' min='0.8' max='1.5' step='0.1' value='1' style='flex:1 1 auto;width:100%'/>
...
<div style="padding:0.5rem 1rem;border-top:1px solid #eee;width:100%;display:flex;flex-flow:row nowrap;justify-content:space-between;">
	...
	<span id='Aside_Pager' style='background:#fff;gap:10px;'>
		<input data-h='page:goto' type='number' min='1' step='1' style='font-size:90%;text-align:right;'/> /
					...
					case "page:goto":
						if (f[1]) {
							...
							document.querySelector(
								`#__${e.parentNode.querySelector('input').value}__`
							).click();
						}
						break;
					case 'addValue':
...
`;
	...
	{	// install TOC table {{{
		...
		.reduce((rs, sec, idx) => {
			...
			if (t) {
				t=t.textContent;
				rs+=`<li data-h="page:goto,${sec.id}">${t}</li>`;
/*
				if (t.nodeType===1)
...
`;
	...
	{	// async init {{{
		...
		// ?xf=quizgen : load extension module from URI
		if (Args.xf)
			await (await loadScript(
					currentScript.getAttribute("src").replace(/\.js/,`_${Args.xf}.js`)
			))(content) ;

		// <div data-x='quiz:...'/> : load extension module from DOM
...
`;
		...
		content.addEventListener('scrollend', (evt) => { // auto activate page when current page out of viewport
	...
	{	// activate specified section {{{
		...
		if (section !== this.current) {
			...
			if (this.Aside)
				this.Aside.update(
					parseInt(/__([0-9]+)__/.exec(section.id)[1]),
	...
	{	// set/unset parameters {{{
		switch (name) {
		case 'mode' :
			Array.from(this.Content.classList).forEach((v)=>{
				if(v.startsWith('mode_')) e.classList.remove(v);
			})
			this.Content.classList.add("mode_"+value);
			break;
		}
	}	// }}}
...
document.addEventListener('DOMContentLoaded', async () => { // {{{
	...
	let MS = window.App = new Pages(document.body);
	await MS.init(args);

	// 3. Install page plugins
